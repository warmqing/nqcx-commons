...
public class LoginTicketUtils {
	...
	public static String generateTickBolob(String random, String account,
			Date issueDate, Date expires, String key) throws Exception {
		return new String(encrypt(
				createTickBolob(random.getBytes(), account.getBytes(ENCODING),
						dateToByteArray(issueDate), dateToByteArray(expires)),
				key));
	}

	private static byte[] createTickBolob(byte[] random, byte[] account,
			byte[] issueDate, byte[] expires) throws Exception {
		if (random.length != 8)
	...
	public static LoginTicket getLoginTicket(String desc, String key)
			throws Exception {
		return parseTickBolob(decrypt(desc, key));
	}

	public static LoginTicket parseTickBolob(byte[] ticketBytes)
			throws Exception {
		...
		ticket = new LoginTicket(_account, null, null, _issueDate, _expires, 0,
				false);
	...
	public static void main(String[] args) {
		...
		try {
			testByte = createTickBolob("123iudf8".getBytes(),
					"nqcx".getBytes(ENCODING),
					LoginTicketUtils.dateToByteArray(new Date()),
					LoginTicketUtils.dateToByteArray(DateUtils.addMinutes(
							new Date(), 30)));

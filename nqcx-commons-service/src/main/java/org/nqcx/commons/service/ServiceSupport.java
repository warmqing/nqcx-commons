...
public abstract class ServiceSupport implements ServiceInterface {
	...
	public <T> DTO add(T t) {
		try {
			...
			return new DTO(affected > 0).putResult("affected", affected);
		} catch (Exception e) {
	...
	public <T> DTO modify(T t) {
		try {
			return new DTO(true).putResult("changed", getDao().update(t));// 如果返回0，表示数据未被修改
		} catch (Exception e) {
	...
	public <T> List<T> query(DTO dto) {
		try {
			...
			dto.setList(getDao().query(parseParams(dto)));
